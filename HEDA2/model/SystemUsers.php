...
//put your code here
  ...
  /*  public function Fetch($set_table_name,$username,$email){
    $bad =0;
    $db = $this->conn;
    $query=mysqli_query($db, "SELECT username,email FROM system_users WHERE username='$username' OR email='$email'");
...
*/
    //funtion to check for duplicate entries for username and email
//please u dont av to pass again the table name and u have used it already in the query
public function fetch($username, $email){
$bad = 0;
$db = $this->conn;
$query = mysqli_query($db, "SELECT username, email FROM system_users WHERE username='$username' OR email = '$email'");
$row = mysqli_fetch_row($query);//mysqli_fetch_row returns NULL if the number of rows is zero else it returns an array of the rows data
if($row != NULL){
$bad = 1;//there is duplicate
}
return $bad;
}
