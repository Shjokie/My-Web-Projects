...
function printFirewoodUsage() {
        $db = $this->conn;
        $result = mysqli_query($db, "SELECT * FROM incoming_messages WHERE deleted=0 AND energy_id=2 ORDER BY date_received DESC ");
        $num = 0;
        $pdf = new FuelDataPDF();
        $pdf->AliasNbPages();
        $pdf->SetFont('Arial', '', 12);
        ...
        $pdf->AddPage();
        $pdf->FuelTable($data);
        $pdf->Output("../files/List of Household Firewood use.pdf");
    }
...
function getTotalCharcoalConsumption() {
        $db = $this->conn;
        $sum = array();
        $result = mysqli_query($db, "SELECT amount FROM incoming_messages WHERE deleted=0 AND energy_id=1 ");
        while ($row = mysqli_fetch_array($result)) {
            $sum[] = $row['amount'];
        }
        return $sum;
    }
    function getTotalFirewoodConsumption() {
        $db = $this->conn;
        $sum = array();
        $result = mysqli_query($db, "SELECT amount FROM incoming_messages WHERE deleted=0 AND energy_id=2 ");
        while ($row = mysqli_fetch_array($result)) {
            $sum[] = $row['amount'];
        }
        return $sum;
    }
    function getTotalKeroseneConsumption() {
        $db = $this->conn;
        $sum = array();
        $result = mysqli_query($db, "SELECT amount FROM incoming_messages WHERE deleted=0 AND energy_id=3 ");
        while ($row = mysqli_fetch_array($result)) {
            $sum[] = $row['amount'];
        }
        return $sum;
    ...
    function getEnergyUsage() {
        ...
        function getKeroseneUsage() {
        $db = $this->conn;
        $result = mysqli_query($db, "SELECT * FROM incoming_messages WHERE deleted=0 AND energy_id=3 ORDER BY date_received DESC ");
        echo "<table width = '880' border='0' class='table_s1' >";
        echo "<th width='10'>#</th>";
        echo '<th>Respondent</th>';
        echo "<th>Fuel</th>";
    ...
    function getCharcoalUsage() {
        $db = $this->conn;
        $result = mysqli_query($db, "SELECT * FROM incoming_messages WHERE deleted=0 AND energy_id=1 ORDER BY date_received DESC ");
        echo "<table width = '880' border='0' class='table_s1' >";
        echo "<th width='10'>#</th>";
        echo '<th>Respondent</th>';
        echo "<th>Fuel</th>";
